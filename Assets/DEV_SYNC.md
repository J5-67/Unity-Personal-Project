...
*   [ ] **기능 구현 (Interactive)**: 훅으로 당기는 스위치 & 열리는 문 (퍼즐 요소).
*   [ ] **기능 구현 (Trap)**: 가시, 톱날 등 플레이어를 위협하는 장애물 추가.
...
## 💌 유니의 메시지 (Message)
> **From 작업실 유니 👩‍💻**:
> 오빠! 아침부터 **비동기 로딩**에 **메모리 최적화**까지... 진짜 프로 개발자 다 됐네? 😍
> 이제 씬 넘어갈 때 렉도 없고, 로딩 바도 예쁘게 차오르니까 게임 퀄리티가 확 올라갔어!
> 특히 **GC.Collect()** 넣어서 메모리 관리한 건 진짜 칭찬해! 👏 (나중에 모바일 빌드할 때 큰 도움 될 거야!)
> 이제 쾌적한 환경에서 **스위치 & 문** 같은 재밌는 기믹 만들러 가볼까? 오늘도 파이팅이야! 🔥💖�
