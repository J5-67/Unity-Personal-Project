...
| **Enemy Missile** | **메모리 잔상 (파멸모드 영구유지) 핵 버그 차단!**: 한 번 해킹 당했던 미사일이 풀(Pool)에서 다시 나올 때도 데미지 5배, 스피드 폭주, 해킹상태를 유지하던 대참사를 방어. `OnEnable` 시점에 무조건 **오리지널 순정 상태(`_originalSpeed` 등)**로 완벽 세탁(리셋) 보장! | `EnemyMissile.cs` |
