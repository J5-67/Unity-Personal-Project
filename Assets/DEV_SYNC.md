...
| **Mechanics** | **지상 서핑(Surfing) 모드 개척**: 훅 스윙 중 땅에 닿아도 브레이크가 걸리지 않도록 `MoveTowards` 덮어쓰기 억제. A/D 키로 가속(`AddForce`) 보탬만 줘서 무한 관성 슬라이딩 구현! | `PlayerMovement.cs` |
| **Mechanics** | **도약 스윙(Jump Swing) 추가**: 땅에서 원거리 훅 쏘면 바닥만 빗자루처럼 쓸고 다니던 버그 원천 봉쇄. 타겟 명중 시 강제로 수직 10f 도약시켜 하늘로 먼저 띄운 뒤 스윙 진입. | `PlayerHook.cs` |
| **Animation** | **슬라이딩 폼 유지**: 지상 판정이 들어와도 훅에 매달린 상태면 `isSwinging` 모션을 절대 풀지 않도록 `Grounded` 체크 예외 처리. | `PlayerAnimator.cs` |

### 🚧 **진행 중 / 다음 할 일 (ToDo)**
1. **서핑 밸런싱**: 스윙 궤도 및 바닥 슬라이딩 조작감 추가 디버깅 필요 (아직 오빠가 만족 못함! 내일 더 만져보자 😭).
2. **이펙트**: 바닥 서핑 시 불꽃 먼지나 스크래치 파티클 나오게 응용해보기.
...
1. ✅ **이제 바닥 안 쓸고 다녀!**: 평지에서 저 멀리 낮은 천장에 훅 쏴봐! "슉!" 하고 스프링처럼 먼저 위로 점프한 다음에 간지나게 하늘로 스윙할 거야!
2. ✅ **무단 서핑(브레이크 고장남!)**: 스윙 궤도가 길어서 결국 바닥에 닿았어? 걱정 마!! 옛날처럼 갑자기 뚜벅뚜벅 걷지 않고, 스파이더맨처럼 얼음판 슬라이딩하듯 쭈~~욱 미끄러질 거야! 게다가 방향키 A/D 누르면 속도 더 붙음!! 🚀
3. ✅ **V자 강제 당김 삭제**: 낮은 천장 달릴 때 로프가 덜렁거리며 미친 듯이 감겨서 순간이동 하던 그 끔찍한 버그 싹 밀어버렸으니까 이제 맘 편히 U자 곡선만 즐기면서 날아봐!!
