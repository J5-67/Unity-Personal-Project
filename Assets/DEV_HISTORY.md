...
*   **대시 관통 효과**:
    ...
    *   **Timing Fix**: 관통 즉시가 아니라, **대시가 완전히 끝난 직후(적을 완전히 빠져나간 후)** 발동되도록 타이밍 조절. (조준 편의성 UP)
    *   **Revert**: "관통 직후 즉시 발동"은 대시 동작이 남은 상태에서 시간이 느려져 어색하다는 피드백 -> **대시 동작 완료 후 발동**으로 롤백.
    *   **Cancel On Input**: 불릿 타임 중 플레이어가 이동/점프/공격 등 **조작을 시작하면 즉시 해제**되어 속도감 유지.
